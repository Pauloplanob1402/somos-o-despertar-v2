 ...
 */
export function MuralDeOracaoView() {
  const { abrirComposer } = useApp();
  const searchParams = useSearchParams();
  const filtroInicial = searchParams.get("filtro");
  const [filtro, setFiltro] = useState<Filtro>(
    filtroInicial === "testemunhos" || filtroInicial === "aguardando" ? filtroInicial : "recentes"
  );
  ...
  const carregar = useCallback(async (f: Filtro) => {
    ...
    // se chegou com #post-xxx na URL (veio do link "ver o testemunho"),
    // rola até ele assim que a lista terminar de carregar.
